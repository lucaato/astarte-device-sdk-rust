...
impl<S> GrpcStore<S> {
    ...
    async fn load_server_properties(&self) -> Result<Vec<StoredProp>, GrpcStoreError>
    where
        S: PropertyStore,
    {
        ...
        match server_props {
            Ok(p) => Ok(p),
            Err(e) => {
                error!(err=%Report::new(e), "error while retrieving server owned properties, returning locally stored server properties");

                self.inner
                    .server_props()
                    .await
                    .map_err(|e| GrpcStoreError::StoreError(StoreError::server_props(e)))
            }
        }
    }
}
    ...
    S: PropertyStore,
{
        ...
        if property.ownership() == Ownership::Server {
            ...
            match property_res {
                ...
                Err(e) => {
                    ...
                    // TODO probably we would still need to store server properties locally otherwise trying to return a server owned property would not make sense here
    ...
    async fn load_all_props(&self) -> Result<Vec<StoredProp>, Self::Err> {
        ...
        let server_props = self.load_server_properties().await?;
    ...
    async fn server_props(&self) -> Result<Vec<StoredProp>, Self::Err> {
        self.load_server_properties().await
    }
