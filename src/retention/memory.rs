...
impl SharedVolatileStore {
    ...
    pub(crate) async fn get_unsent(
        &mut self,
        buf: &mut Vec<(Id, ItemValue)>,
        limit: usize,
    ) -> usize {
        self.store.lock().await.get_unsent(buf, limit)
    }

    pub(crate) async fn reset_sent(&mut self) {
...
impl VolatileStore {
    ...
    fn get_unsent(&mut self, unsent: &mut Vec<(Id, ItemValue)>, limit: usize) -> usize {
        ...
        let unsent_iter = self
            .store
            .iter()
            .filter_map(|item| (!item.sent).then_some((item.id, item.value.clone())))
            .take(limit);
